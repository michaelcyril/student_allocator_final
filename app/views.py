...
def specialSchool(year):
    ...
    # for girls
    special_school_f = School.objects.values('id', 'name', 'sex', 'type').filter(type="special", sex="female")
    ...
    students_f = Student.objects.values('id', 'candidate_name', 'sex').filter(
        Q(year=year) and Q(is_active=True) and Q(sex="female")).order_by(
        '-average_marks')[:total_required_special_f]
    ...
    # for boys
    special_school_m = School.objects.values('id', 'name', 'sex', 'type').filter(type="special", sex="male")
    ...
    students_m = Student.objects.values('id', 'candidate_name', 'sex').filter(
        Q(year=year) and Q(is_active=True) and Q(sex="male")).order_by(
        '-average_marks')[:total_required_special_m]
    ...
    output_list_m = [{'id': d['id'], 'name': d['name'], 'sex': d['sex'],
                      'type': d['type'], 'quantity_required': d['quantity_required'],
                      'students': sub_list} for d, sub_list in zip(ssm_list, student_groups_m)]
    # This is to save the students to the special school
...
def technicalSchool(year):
    # This give us required student in special schools
    # for girls
    special_school_f = School.objects.values('id', 'name', 'sex', 'type').filter(type="technical", sex="female")
    ssf = [x for x in special_school_f]
    ssf_list = []
    total_required_special_f = 0
    for d in ssf:
    ...
    # for boys
    special_school_m = School.objects.values('id', 'name', 'sex', 'type').filter(type="technical", sex="male")
    ssm = [x for x in special_school_m]
    ssm_list = []
    total_required_special_m = 0
    for d in ssm:
